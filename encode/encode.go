...
// Uint16Bytes converts the uint16 to a length-2, big-endian encoded byte slice.
...
// Uint32Bytes converts the uint32 to a length-4, big-endian encoded byte slice.
...
// GobEncode encodes the thing to []byte.
func GobEncode(thing interface{}) ([]byte, error) {
	var buf bytes.Buffer
	if err := gob.NewEncoder(&buf).Encode(thing); err != nil {
		return nil, fmt.Errorf("Encoding of %T failed: %w", thing, err)
	}
	return buf.Bytes(), nil
}

// GobDecode decodes the []byte into the thing.
func GobDecode(b []byte, thing interface{}) error {
	return gob.NewDecoder(bytes.NewReader(b)).Decode(thing)
}
